...
# In the container, edit /home/onyx/.openclaw/openclaw.json:
...
```
OPENCLAW_TOKEN=<new-strong-token>
OPENCLAW_MODEL=<whatever-onyx-uses>
```

(Get the model name via `docker exec onyx cat /home/onyx/.openclaw/openclaw.json | jq .gateway` — look at the default model Onyx is configured for.)
...
| Chat | `OPENCLAW_TOKEN` in systemd env file (0600, root-owned), not in code |
