...
async function getDisplayedToken(page) {
  ...
  return await page.locator('eh-settings-security .hae-token-value .endpoint-code').innerText();
}
...
test.describe('#278 HAE token Settings flow', () => {
  test('generate, copy, ingest, regenerate rotates', async ({ page, request, baseURL }) => {
    ...
    // HAE lives in the Security tab now.
    ...
    const generateBtn = page.locator('eh-settings-security .hae-token-empty button', {
    ...
    await expect(page.locator('eh-settings-security .hae-token-value')).toBeVisible();
    ...
    await page.locator('eh-settings-security .hae-token-value button', { hasText: /Regenerate/i }).click();
    const confirmBtn = page.locator('eh-settings-security .hae-regen-actions button', {
