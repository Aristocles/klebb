...
const DOC_INDEX = [
  ...
  { kind: 'source', path: 'public/js/components/eh-schedule-card.js',
    summary: 'schedule-card renderer. Default ✓ check-off HARDCODES `{scheduledDate, takenAt, offSchedule?}` to data.items[].doses[]. With meta.view.checkOffForm set (currentDoseFields + previousDoseFields, optional previousDosePrompt + currentDosePrompt for section headings), ✓ instead opens an inline form sourced from meta.writeable.inputs and merges previous-dose fields onto the most recent prior taken dose (retroactive review). Re-tapping ✓ on a logged date pre-fills the form for editing. The viewed date\'s logged values surface as a muted summary line on the item (chips-multi `none` is filtered). Renders a time-of-day chip (☀️/🌤️/🌙/💤) next to the row when `schedule.time_of_day` is set (single token or array, drawn from morning|midday|evening|night); presence is the toggle, no view-config opt-out. With meta.writeable.maxReadingsPerDay > 1 each check-off STACKS a fresh doses[] entry (up to the cap, no-op beyond it) instead of toggling, so one date can hold several entries; a bare doses[scheduledDate=...] row filter is then AMBIGUOUS. The per-item dose text obeys view.doseLine ("scheduled" default / "always" / "never"). Reads data.items[].{schedule, cycles[], doses[]} and view.colorMap.' },
  { kind: 'source', path: 'public/js/components/eh-checklist-card.js',
    summary: 'checklist-card renderer. HARDCODES the daily-tick write to either item.doses[].push({scheduledDate, takenAt}) or item.takenDates[].push(date); DOES NOT consult meta.writeable.inputs. With meta.writeable.maxReadingsPerDay > 1 the doses[] shape stacks one entry per tap up to the cap (takenDates shapes ignore the flag). Accepts data shapes items[], {items}, or {current}. Renders a time-of-day chip (☀️/🌤️/🌙/💤) next to the row when `schedule.time_of_day` is set, via the same shared helper as schedule-card.' },
