...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    function: {
      name: 'read_report',
      description:
        "Fetch the full text of one of the user's uploaded reports. The system prompt lists every report under '## Available reports' with its name, title and document date; pass one of those names verbatim (no .md extension, no slashes). Reports are text extracted from PDFs, photos, scans, Word documents, notes or audio the user has uploaded: blood panels, scan reports, doctors' letters, voice memos and similar. Use this when the user asks 'what does my latest blood panel show', 'summarise the MRI report', or any other question where the answer is in one of those documents, and ALWAYS before quoting a specific figure (the catalogue holds digests, not the document). Returns {name, path, content, title, documentDate, ingestedAt, sourceFormat, status, verify, truncated}; unknown or invalid names return {error}. A report whose text came from OCR and has not been checked by the user returns {error, verify:'required'} and NO content: when that happens, tell the user the report is waiting to be verified on the Reports page and do not guess at what it contains.",
      parameters: {
        type: 'object',
        properties: {
          name: {
