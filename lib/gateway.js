...
// come back usefully.
const GATEWAY_HARD_TIMEOUT_MS = 540000;

// A timeout is only *soft* (rejects as gateway_iter_timeout, which the agent
// loop catches and reports as a capped turn) while it is strictly under the
// ceiling; at or above it, the same stall rejects as a hard gateway_timeout
// and the whole turn is lost to a 504. So a caller that wants soft behaviour
// cannot just pass its own budget: CHAT_ITER_TIMEOUT_MS=0 means "no per-step
// cap" and CHAT_TURN_DEADLINE_MS can legitimately exceed the ceiling, and
// either sends a budget through that is not soft at all (#694). Clamping here
// rather than at the call site keeps the rule with the number it depends on.
const SOFT_CAP_MARGIN_MS = 1000;

function softStepBudget(budgetMs) {
  if (!(typeof budgetMs === 'number' && budgetMs > 0)) return budgetMs;
  return Math.min(budgetMs, GATEWAY_HARD_TIMEOUT_MS - SOFT_CAP_MARGIN_MS);
}
...
// tolerated, GATEWAY_HARD_TIMEOUT_MS per-hop ceiling).
//
...
module.exports = {
  callGateway, isConfigured, parseEndpoint, GATEWAY_HARD_TIMEOUT_MS, softStepBudget,
