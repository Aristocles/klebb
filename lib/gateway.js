...
function createDeltaAssembler(onDelta) {
  const acc = { content: '', toolCalls: [], finishReason: null, sawChunk: false };
  return {
    take(parsed) {
    ...
    result() {
      ...
      return { choices: [{ finish_reason: acc.finishReason, message }] };
    },
  };
}
...
function callGateway({ messages, tools, timeoutMs, endpointUrl, model, apiKey, stream, onDelta } = {}) {
