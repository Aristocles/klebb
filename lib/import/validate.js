...
//   const { ok, findings, plan } = await validateTree(treePath, { targetHome });
//
...
// Chunked on purpose: the inventory hashes every listed file including
// samples.json, and a real history file can be bigger than a small
// container's heap (#639).
...
// First non-whitespace character of a file, without reading it whole.
function firstNonWs(file) {
  const fd = fs.openSync(file, 'r');
  try {
    ...
    for (;;) {
      const n = fs.readSync(fd, buf, 0, buf.length, null);
      if (n === 0) return null;
      const m = buf.toString('utf8', 0, n).match(/\S/);
      if (m) return m[0];
    }
  } finally {
    fs.closeSync(fd);
...
async function validateTree(treePath, opts = {}) {
  const findings = [];
  ...
  if (hasDataDir) {
    ...
    if (fs.existsSync(samplesAbs)) {
      // Streamed: the plan needs counts, not the pushes, and a real history
      // file can be bigger than a small container's heap even inside the
      // per-file cap (#639). One parsed element lives at a time; header
      // values arrive via the reader's capture hook so the version gate
      // holds without a whole-file parse. A bare top-level array is the
      // legacy drain shape and has never validated here, so anything but an
      // object refuses on the first byte.
      let shapeOk = true;
      let versionOk = true;
      let pushes = 0;
      let empty = 0;
      if (firstNonWs(samplesAbs) !== '{') {
        shapeOk = false;
      } else {
        const onHeader = (key, v) => {
          if (key !== 'version') return;
          if (v.kind === 'complex') { versionOk = false; return; }
          try {
            versionOk = JSON.parse(v.text) === 1;
          } catch {
            versionOk = false;
          }
        };
        try {
          for await (const p of streamPushes(samplesAbs, { onHeader })) {
            pushes += 1;
            if (!p || !p.payload) empty += 1;
          }
        } catch {
          shapeOk = false;
        }
      ...
      if (!shapeOk || !versionOk) {
        ...
        if (pushes > caps.maxPushes) {
          findings.push(finding('refusal', 'samples', SAMPLES_FILE, 'VAL_CAP',
            `samples.json has ${pushes} pushes; the cap is ${caps.maxPushes} (KLEBB_IMPORT_MAX_PUSHES)`));
        }
        if (empty > 0) {
        ...
        plan.samplesPushes = pushes - empty;
      }
    }
