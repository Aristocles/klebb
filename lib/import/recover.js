...
//
//   const { action } = await recoverAtBoot({ home, ...deps });
...
async function recoverAtBoot(opts) {
  const { home } = opts;
  ...
  if (record.treePath && fs.existsSync(record.treePath)) {
    return { action: 'resumed', source: 'tree', result: await wizard.resume('tree') };
  }
  if (record.snapshotPath && fs.existsSync(record.snapshotPath)) {
    return { action: 'resumed', source: 'snapshot', result: await wizard.resume('snapshot') };
  }
  return {
    action: 'refuse',
