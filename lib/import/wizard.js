...
//   await wizard.confirmAndApply({ nonce })  // runs the pipeline; retry from 'failed'
//   await wizard.rollback()             // failed|done -> re-apply the snapshot
//   wizard.abort()                      // awaiting-confirm|failed|done -> clear
//
// confirmAndApply/rollback/resume are async (#632: the samples drain
// streams, with event-loop yields). The pipeline is still one job at a
// time by construction: the state moves to 'applying' before the first
// await, so a second call in the window answers BAD_STATE/JOB_ACTIVE.
//
...
function createWizard(deps) {
  ...
  async function confirmAndApply({ nonce } = {}) {
