...
//   await wizard.startFromTree(treePath) // -> status, or {error, code:'JOB_ACTIVE'}
...
//
// The apply pipeline is wipe-first by construction: engage the write freeze
// (before its first await, so nothing writes once 'applying' is answered),
// re-validate, snapshot (populated targets only), quiesce the watcher, WIPE
...
function createWizard(deps) {
  ...
  function publicStatus() {
    ...
    // The selectable inventory and what the target holds today are inputs to
    // one decision, so they ride only the status the caller decides from. The
    // apply polls run every 1.5s and would carry an entry per card for
    // nothing; once confirmed, the selection is settled and cannot change.
    if (job.state === 'awaiting-confirm') {
      s.items = job.items || null;
      s.target = job.target || null;
    }
  ...
  // The guard and the job assignment run before the first await, so a
  // concurrent second start always sees the active job and refuses.
  async function startFromTree(treePath) {
    ...
    const validation = await validateTree(job.treePath, { targetHome: home });
  ...
  // Remove a directory, keeping any spared path (and the chain of parents
  // above it) intact. A dir holding no spared path goes whole.
  function rmDirSparing(dir, spare) {
    const keepsAny = [...spare].some(s => s === dir || s.startsWith(dir + path.sep));
    if (!keepsAny) {
    ...
    for (const ent of entries) {
      ...
      // Spares are directories by contract (raw-archives.js); a same-named
      // FILE is exported like any other and must be wiped like any other.
      if (ent.isDirectory()) {
        if (spare.has(p)) continue;
        rmDirSparing(p, spare);
      } else {
  ...
  function wipe() {
    ...
    // The one exception to the directory sweep: the raw push archives under
    // auto-export/ are excluded from every export and every rollback
    // snapshot (they are hundreds of MB of superseded duplicates), so a
    // wipe that removed them would destroy the only copy with nothing able
    // to restore it (#656). The shared list keeps this in lockstep with
    // the export's skips.
    const spare = new Set(rawArchiveDirs(path.join(dataDir, 'auto-export')));
    for (const ent of entries) {
      // Every other directory under data/ is previous-instance state that
      // the export carries verbatim and the copy restores: auto-export/
      // (HAE inbox, discovered metrics, quarantined payloads), _archive/ (a
      ...
      if (ent.isDirectory()) {
        rmDirSparing(path.join(dataDir, ent.name), spare);
        continue;
      }
  ...
  async function runPipeline(source) {
    ...
    try {
      // (b) Gate writes before anything else. The re-validate below is the
      // pipeline's first await (#639 streams the samples scan), so the freeze
      // must engage before the first yield to the event loop, not merely
      // before the wipe: a write landing after validation started would
      // otherwise mutate a target the snapshot has yet to capture, and be
      // recoverable from neither the snapshot nor the wiped target.
      stage('freeze');
      ...
      const validation = await validateTree(root, { targetHome: home });
      ...
      if (source === 'tree' && job.fresh === false && !job.snapshotPath && firstWipe) {
        ...
        // silently break ingest. Strict: a samples-export failure must fail
        // the pipeline HERE, before the wipe, not produce a snapshot that
        // validates clean with no history (#672).
        exportTo(snapDir, { includeSecrets: true, strict: true });
      ...
      // (c) Quiesce the watcher so a queued debounce reload cannot fire
      // mid-wipe. The write gate (b) engaged at the top of the pipeline.
      registry.stopWatch();
      watchStopped = true;
      ...
      // The freeze (b) engaged before the pipeline's first await (the
      // re-validate), so a request landing in a drain yield can read but
      // not write, and the finally below releases across every await path.
      stage('drain');
