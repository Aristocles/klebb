...
// How long to wait for a database another process is writing to. Generous on
// purpose: every writer here holds the lock for a single short transaction, so a
// wait this long only ever happens under real contention, and the alternative is
// an immediate hard failure. Matches health-auto-export/samples.js, which opens
// the same file with its own handle.
...
function open(dbFile = PATHS.DB_FILE) {
  ...
  // Wait for a busy database instead of failing instantly. SQLite's default
  // busy timeout is 0, so a second opener that finds the write lock held throws
  // SQLITE_BUSY ("database is locked") straight away.
  //
  // Set BEFORE journal_mode, because journal_mode is the statement that fails:
  // switching a database into WAL takes the write lock, so the very first exec
  // of a fresh open is the one that collides. Measured across two processes, one
  // holding a write for 600ms: without this, the opener fails after 24ms; with
  // it, the opener waits 141ms and succeeds.
  //
  // This is not only a test concern. scripts/export-embed.js deliberately runs
  // against a live instance (that is how Cloud exports a running container), so
  // the same collision is a failed customer export rather than a red CI run.
  ...
  // Drop every card in one transaction, then clear the in-memory maps so the
  // served values vanish with the rows. In-place SQL on purpose: never unlink
  // the db files to wipe, because a live server keeps its handle open and
  // would carry on writing to the deleted inode while a fresh open creates a
  // new file nobody else can see. Returns the number of cards removed.
  function wipeAll() {
    const removed = _values.size;
    db.exec('BEGIN');
    try {
  ...
  return { load, getData, setData, hasData, dataUpdatedAt, deleteCard, wipeAll, close, file: dbFile };
}
