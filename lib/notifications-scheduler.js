...
const reminders = require('./reminders-helper');
    ...
    for (const item of items) {
      ...
      if (item.trigger.type === 'schedule_due') {
        ...
        if (target && targetItems && targetEnabled) {
          const todayIso = reminders.todayIsoIn(now, tz);
          const filtered = reminders.filterScheduleDue(scheduleLib, target, item.trigger.time_of_day, todayIso);
