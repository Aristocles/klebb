...
// Intents that mark the line as a non-report. Triage found half the
// collected log was placeholder lines the model wrote when it decided
// mid-turn there was nothing to report after all (#724). Two tiers:
// tokens no real report ever STARTS with (placeholder, n/a, accidental,
// ignore) reject as prefixes; words that can legitimately open a real
// report ("skip button does nothing", "test results table misaligns")
// reject only when they are the whole intent.
const PLACEHOLDER_RE = /^(placeholder|n\/?a|accidental|ignore)\b|^(skip|noop|none|test|testing)[\s.!:-]*$/i;
...
function appendFeedback(input, nowIso) {
  ...
  if (PLACEHOLDER_RE.test(line.intent)) {
    return { logged: false, reason: 'placeholder intent; log only a real gap, or do not call at all' };
  }
  try {
