...
const remindersHelper = require('./reminders-helper');
...
function buildPayload(ev) {
  ...
  const items = ev.items.map(i => {
    ...
    // Structured carry-forward for the reminder modal. null when the
    // trigger has no item structure (daily/weekly) or when nothing is
    // due — the SW uses null to decide whether to surface the modal.
    const surv = Array.isArray(i.surviving) ? i.surviving : [];
    const miss = Array.isArray(i.missed_earlier) ? i.missed_earlier : [];
    const reminders = (surv.length || miss.length)
      ? { due_now: surv, missed_earlier: miss }
      : null;
    return {
      id: i.id,
      time: hhmm,
...
// from the manifest item directly and send to every live sub. For
// schedule_due items, compute surviving/missed_earlier against the
// target card so the test-fired notification carries the same modal
// shape a real fire would.
async function testFire({ manifest, item, slot, targetCard }) {
  const slotIso = slot || new Date().toISOString();
  let surviving = [];
  let missedEarlier = [];
  if (item?.trigger?.type === 'schedule_due' && targetCard?.data?.items) {
    const tz = userTz.readUserTz();
    const scheduleLib = await _loadScheduleLib();
    const todayIso = remindersHelper.todayIsoIn(new Date(), tz);
    const filtered = remindersHelper.filterScheduleDue(
      scheduleLib, targetCard, item.trigger.time_of_day, todayIso,
    );
